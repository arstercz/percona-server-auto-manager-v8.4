...
static RuleSet rule[] = {
  ...
  {
    {
      {(char*)"^\\s*CREATE\\s+INDEX\\s+", RTRUE},
    },
    (char*)"disabled create index syntax.",
  },
  {
    {
      {(char*)"^\\s*ALTER\\s+.*(DROP|CHANGE|MODIFY|RENAME)\\s+", RTRUE},
    },
  ...
  {
    {
      {(char*)"^\\s*(DROP\\s+DATABASE|DROP\\s+SCHEMA)\\s+", RTRUE},
    },
    (char*)"disabled drop database syntax.",
  },
  {
    {
      {(char*)"^\\s*(PURGE|TRUNCATE|DROP\\s+TABLE)\\s+", RTRUE},
    },
    (char*)"disabled descreased DDL syntax.",
  },
  {
    {
      {(char*)"^\\s*REVOKE\\s+", RTRUE},
    },
    (char*)"disabled revoke syntax.",
  },
  {
    {
      {(char*)"^\\s*GRANT\\s+ALL\\s+", RTRUE},
    },
